  ...
  sdf::ElementPtr sdf)
{
  ...
  gazebo::physics::PhysicsEnginePtr physics =
    gazebo::physics::get_world()->Physics();
  ...
  if (this->dataPtr->n_dof_ == 0 && this->dataPtr->n_sensors_ == 0) {
    RCLCPP_WARN_STREAM(
      this->nh_->get_logger(),
      "There is no joint or sensor available");
    return false;
  }
  ...
  gazebo::physics::ModelPtr parent_model)
{
  ...
  for (unsigned int j = 0; j < n_sensors; j++) {
    ...
    // This can't be used, because it only looks for sensor in links, but
    // force_torque_sensor must be in a joint, as in not found by
    // SensorScopedName std::vector<std::string> gz_sensor_names =
    // parent_model->SensorScopedName(sensor_name);

    // Workaround to find sensors whose parent is a link or joint of
    // parent_model
    std::vector<std::string> gz_sensor_names;
    for (const auto & s :
      gazebo::sensors::SensorManager::Instance()->GetSensors())
    {
    ...
    if (gz_sensor_names.empty()) {
      RCLCPP_WARN_STREAM(
        this->nh_->get_logger(),
        "Skipping sensor in the URDF named '" <<
          sensor_name <<
          "' which is not in the gazebo model.");
      continue;
    }
    ...
    gazebo::sensors::SensorPtr simsensor =
      gazebo::sensors::SensorManager::Instance()->GetSensor(
      gz_sensor_names[0]);
    if (!simsensor) {
      RCLCPP_ERROR_STREAM(
        this->nh_->get_logger(),
        "Error retrieving sensor '" <<
          sensor_name << " from the sensor manager");
      continue;
    }
    if (simsensor->Type() == "imu") {
      ...
      if (!imu_sensor) {
        RCLCPP_ERROR_STREAM(
          this->nh_->get_logger(),
          "Error retrieving casting sensor '" <<
            sensor_name << " to ImuSensor");
        continue;
      }
      ...
      gazebo::sensors::ForceTorqueSensorPtr ft_sensor =
        std::dynamic_pointer_cast<gazebo::sensors::ForceTorqueSensor>(
        simsensor);
      if (!ft_sensor) {
        RCLCPP_ERROR_STREAM(
          this->nh_->get_logger(),
          "Error retrieving casting sensor '" <<
            sensor_name << " to ForceTorqueSensor");
        continue;
      }
  ...
  this->dataPtr->imu_sensor_data_.resize(
    this->dataPtr->sim_imu_sensors_.size());
  ...
  for (unsigned int i = 0; i < imu_components_.size(); i++) {
    ...
    for (const auto & state_interface : imu_components_[i].state_interfaces) {
      ...
      this->dataPtr->state_interfaces_.emplace_back(
        sensor_name, state_interface.name,
  ...
  for (unsigned int i = 0; i < ft_sensor_components_.size(); i++) {
    ...
    RCLCPP_INFO_STREAM(
      ...
      "Loading sensor: " << sensor_name);
    RCLCPP_INFO_STREAM(this->nh_->get_logger(), "\tState:");
    for (const auto & state_interface :
      ft_sensor_components_[i].state_interfaces)
    {
      static const std::map<std::string, size_t> interface_name_map = {
...
hardware_interface::return_type GazeboSystem::read()
{
  for (unsigned int j = 0; j < this->dataPtr->joint_names_.size(); j++) {
    if (this->dataPtr->sim_joints_[j]) {
      ...
      this->dataPtr->joint_effort_[j] =
        this->dataPtr->sim_joints_[j]->GetForce(0u);
    }
  }
...
hardware_interface::return_type GazeboSystem::write()
{
  ...
  gazebo::common::Time gz_time_now =
    this->dataPtr->parent_model_->GetWorld()->SimTime();
