...
class fwsKinematics(object):
    ...
    def kin_model(self):
        ...
        beta = np.arctan((self.l_f * np.tan(-delta_f) + self.l_r * np.tan(delta_f))\
                        / (self.l_f + self.l_r))
        v = (v_f * cos(delta_f) + v_r * cos(-delta_f)) / (2 * cos(beta))
        # righthand-side function
        RHS = ca.vertcat(v*cos(psi + beta), v*sin(psi + beta),
                         v * (np.tan(delta_f) * cos(beta) \
                          - np.tan(-delta_f) * cos(beta)) / (self.l_f + self.l_r),
                         virtual_v)
        ## to change
    ...
    # Weighing Matrices
        ...
        R = ca.diagcat(R_vf, R_vr, R_deltaf, R_virtv)

        #rate changing matrix
        W = ca.diagcat(W_vf, W_vr, W_deltaf, W_virtv)
    ...
    def cost_function(self):
        ...
        for k in range(self.N):
            st = X[:, k]
            con = U[:, k]
            vf = U[0,k]; vr = U[1,k]; d_f = U[2,k]; d_r = - d_f
    ...
    def constraints(self, n_states, n_controls, N):
        ...
        ubx[n_states*(N+1)+2:n_states*(N+1)+n_controls*N:n_controls] = delta_max                                                                      # v upper bound for all V
        ...
        ubg[n_states*(N+1)+2:n_states*(N+1)+n_controls*N:n_controls] = w_max*0.3                                                                             # v upper bound for all V
    ...
    def solve_mpc(self, solver, state, args, n_states, n_controls, x_r, up0):
        ...
        delta_f = float(inp[2]); delta_r = - float(inp[2])
        alpha = 0.0; v_virt = float(inp[3])
