...
class Pivot4wsKinematics(object):
    ...
    def cost_function(self):
        ...
        g2 = U[:, 0] - P[-n_controls:]
        g3 = (U[2,0]- P[10]) + (- P[9] + P[8]*cos(P[10]))*sin(P[10])/(self.l_f+self.l_r*cos(P[10]))
        ...
        for k in range(self.N):
            ...
            beta = np.arctan(( self.l_r * sin(U[2,k])) / (self.l_f + self.l_r * cos(U[2,k])))
            if k == (self.N-1): 
                g2 = ca.vertcat(g2, U[:,k] - U[:,k])
                #g3 = ca.vertcat(g3, U[1,k] - U[0,k] * cos(U[2,k]))
                #g3 = ca.vertcat(g3, (U[2,k]- U[2,k]) + (U[1,k]-U[0,k]*cos(U[2,k]))*
                #                (sin(U[2,k]) / (self.l_r * cos(U[2,k]) + self.l_f)
                #              +  1 / (self.l_r)))
                g3 = ca.vertcat(g3, (U[2,k]- U[2,k]) + (- U[1,k] + U[0,k]*cos(U[2,k]))*sin(U[2,k]-beta)/(self.l_f*cos(beta)))
            else: 
                g2 = ca.vertcat(g2, U[:,k] - U[:,k+1])
                #g3 = ca.vertcat(g3, U[1,k] - U[0,k] * cos(U[2,k]))
                #g3 is the constraint to have a relation bw alpha dot and the rear wheel velovity
                #g3 = ca.vertcat(g3, (U[2,k]- U[2,k+1]) + (U[1,k]-U[0,k]*cos(U[2,k]))*
                #                (sin(U[2,k]) / (self.l_r * cos(U[2,k]) + self.l_f)
                #              +  1 / (self.l_r)))
                g3 = ca.vertcat(g3, (U[2,k+1]- U[2,k]) + (- U[1,k] + U[0,k]*cos(U[2,k]))*sin(U[2,k]-beta)/(self.l_f*cos(beta)))
                                
#                g3 = ca.vertcat(g3, U[4,k]- U[4,k+1] + U[1,k]*
#                                (sin(U[4,k] - U[3,k]) / (self.l_r * cos(U[4,k]) + self.l_f)
#                              +  cos(U[4,k] - U[3,k]) / (self.l_r * cos(U[:,4]))))


        obj = obj - eps/2*(st[3])**2 + gamma/2*(st[2] - P[6])**2
        

        g = ca.vertcat(g1, g2[:- n_controls],g3[:-1])
    ...
    def constraints(self, n_states, n_controls, N):
        ...
        lbg = ca.DM.zeros((n_states*(N+1)+n_controls*N + N, 1))  # constraints lower bound
        ubg = ca.DM.zeros((n_states*(N+1)+n_controls*N + N, 1))  # constraints upper bound
        ...
        #constraint on alpha dot
        lbg[n_states*(N+1)+n_controls*N:n_states*(N+1)+n_controls*N + N:1] = 0
        ubg[n_states*(N+1)+n_controls*N:n_states*(N+1)+n_controls*N + N:1] = 0
