...
%% loading
input_path = fullfile(pwd, '../../../4ws/sym/input.txt'); % Paths Folder
load(input_path)
bicycle_path = fullfile(pwd, '../../../4ws/sym/input_bicycle.txt'); % Paths Folder
load(bicycle_path)
state_path = fullfile(pwd, '../../../4ws/sym/state.txt'); % Paths Folder
load(state_path)
prediction_path = fullfile(pwd, '../../../4ws/sym/prediction.txt'); % Paths Folder
pred = load(prediction_path);
reference_path = fullfile(pwd, '../../../4ws/sym/reference.txt'); % Paths Folder
ref = load(reference_path);
path_path = fullfile(pwd, '../../../../path/new_path.txt'); % Paths Folder
path = load(path_path);

load(fullfile(pwd, '../../../4ws/sym/time_step.txt'))
load(fullfile(pwd, '../../../4ws/sym/steer_effort.txt'))
load(fullfile(pwd, '../../../4ws/sym/wheel_effort.txt'))
%% data
N = 10;
dt = 0.1;
lf = 0.16; lr = 0.71;
...
%bicycle
v_f = movmean(input_bicycle(:,1),coeff);
v_r = movmean(input_bicycle(:,2),coeff);
delta_f = movmean(input_bicycle(:,3),coeff);
delta_r = movmean(input_bicycle(:,4),coeff);
...
grid on; xlim([0, time(end)]); ylim([-1.9,1.9]);
...
E_t_st = sum(E_st)
figure
for i = 1 : size(P_st,2) -1
    plot(time, P_st(:,i))
    hold on
end
legend('\delta_{fl}','\delta_{fr}','\delta_{rl}','\delta_{rr}', 'Orientation','horizontal')
