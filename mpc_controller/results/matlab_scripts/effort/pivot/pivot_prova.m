...
%% loading
input_path = fullfile(pwd, '../../../pivot/prova/input.txt'); % Paths Folder
load(input_path)
bicycle = fullfile(pwd, '../../../pivot/prova/input_bicycle.txt'); % Paths Folder
load(bicycle)
state_path = fullfile(pwd, '../../../pivot/prova/state.txt'); % Paths Folder
load(state_path)
prediction_path = fullfile(pwd, '../../../pivot/prova/prediction.txt'); % Paths Folder
pred = load(prediction_path);
reference_path = fullfile(pwd, '../../../pivot/prova/reference.txt'); % Paths Folder
ref = load(reference_path);
path_path = fullfile(pwd, '../../../../path/new_path.txt'); % Paths Folder
path = load(path_path);
load(fullfile(pwd, '../../../pivot/prova/steer_effort.txt'))
load(fullfile(pwd, '../../../pivot/prova/wheel_effort.txt'))
...
figure
subplot(311)
plot(time, v_f, "b", "LineWidth", line_width); 
grid on; xlim([0, time(end)]); ylim([-0.2, 1.2]);
xlabel('time (s)'); ylabel('v_{f} (m/s)')
subplot(312)
plot(time, v_r, "r", "LineWidth", line_width); 
grid on; xlim([0, time(end)]); ylim([-0.2, 1.2]);
...
legend('v_{fl}', 'v_{fr}', Orientation='horizontal')
subplot(212)
plot(time, v_rl, "b", "LineWidth", line_width); 
grid on; xlim([0, time(end)]); ylim([-0.2, 1.2]);
xlabel('time (s)'); ylabel('v_{r} (m/s)')
hold on
plot(time, v_rr, "r", "LineWidth", line_width); 
grid on; xlim([0, time(end)]); ylim([-0.2, 1.2]);
xlabel('time (s)'); ylabel('v_{r} (m/s)')
legend('v_{rl}', 'v_{rr}', Orientation='horizontal')
...
line_width = 1;
figure(10)
plot(t_torque, steer_effort(:,5), "b", "LineWidth", line_width); 
...
subplot(221)
plot(t_torque, wheel_effort(:,1), "b", "LineWidth", line_width); 
grid on; xlim([0, time(end)]);
xlabel('time (s)'); ylabel('T_{fl} (Nm)')
subplot(222)
plot(t_torque, wheel_effort(:,2), "b", "LineWidth", line_width); 
grid on; xlim([0, time(end)]); 
xlabel('time (s)'); ylabel('T_{fr} (Nm)')
subplot(223)
plot(t_torque, wheel_effort(:,3), "b", "LineWidth", line_width); 
grid on; xlim([0, time(end)]);
xlabel('time (s)'); ylabel('T_{rl} (Nm)')
subplot(224)
plot(t_torque, wheel_effort(:,4), "b", "LineWidth", line_width);
grid on; xlim([0, time(end)]); 
...
%% Actual energy computation (R*I^2 + T*omega)
%steer
tk = 0.05; %Nm/A torque constant
V = 80;
st_eff_new = interp1(t_torque', steer_effort(:,5), time);
